...
// Condense a full prescription ("4 × 8-12 @ RPE 8, rest 90s") down to just the
// volume ("4 × 8-12") for list views. Full detail lives in Gym Mode.
function shortDetail(detail = "") {
  let s = String(detail).trim();
  if (!s) return "";
  s = s.replace(/[,;]?\s*rest[^,;]*/i, "");
  s = s.replace(/[,;]?\s*@?\s*rpe\s*[\d.\u2013\-]+/i, "");
  s = s.replace(/\s*\([^)]*\)/g, "");
  s = s.replace(/\b(each|per)\s+side\b/gi, "");
  s = s.replace(/\bsets?\b/gi, "").replace(/\breps?\b/gi, "");
  s = s.replace(/(\d)\s*[x\u00d7]\s*(\d)/i, "$1\u00d7$2").replace(/(\d)\s+of\s+(\d)/i, "$1\u00d7$2");
...
// Feed the actual logged lifts to Claude for a more nuanced read than the
// bodyweight-ratio heuristic can give — it sees exercise selection, rep
// ranges, progression over time and blind spots (what's NOT being trained).
async function aiStrengthRead(p, lifts, groups) {
  const rows = [];
  for (const [k, hist] of Object.entries(lifts || {})) {
    if (!hist || !hist.length) continue;
    const best = Math.max(...hist.map((h) => e1RM(h.sets)));
    ...
    const first = hist[0];
    const firstTop = first.top || bestSet(first.sets);
    const trend = hist.length > 1 && firstTop ? `, first logged ${firstTop.w}×${firstTop.reps}` : "";
    rows.push(`${k}: ${hist.length} session(s), best est 1RM ${Math.round(best)} lb, latest top set ${top.w}×${top.reps}${trend}`);
  }
  if (rows.length < 3) throw new Error("not enough data");
  const gs = Object.entries(groups || {}).filter(([, v]) => v != null).map(([m, v]) => `${m} ${v}`).join(", ");
  const style = p.trainingStyle ? `\nTrainee's own notes: ${p.trainingStyle}` : "";
  const goal = p.goal === "recomp" ? "lose fat while building muscle" : (GOALS[p.goal]?.label || "general fitness");
  const out = await callClaude(
    [{ role: "user", content:
      `Bodyweight: ${p.weightLb || "unknown"} lb. Goal: ${goal}.\n\nLogged lifts:\n${rows.join("\n")}\n\nApp's rough bodyweight-normalized group scores (0-100): ${gs || "n/a"}${style}\n\nAssess this trainee's strength balance.` }],
    'You are an experienced strength coach reading a trainee\'s actual training log. Consider exercise selection, relative strength between antagonist groups (push/pull, quad/hamstring), rep ranges, progression over time, and blind spots — muscle groups with little or no logged work. Be specific and reference their actual lifts and numbers. Reply with ONLY a JSON object, no prose, no markdown: {"summary":string,"strengths":[string],"gaps":[string],"actions":[string]}. "summary" is 1-2 sentences. Each array holds 2-3 short, concrete items (max 18 words each). "actions" are specific things to add or change in training. Do not give medical advice.',
    900);
  const j = parseLoose(out);
  if (!j || !j.summary) throw new Error("bad");
  const arr = (x) => (Array.isArray(x) ? x.filter((s) => typeof s === "string").slice(0, 3) : []);
  return { summary: String(j.summary).slice(0, 400), strengths: arr(j.strengths), gaps: arr(j.gaps), actions: arr(j.actions), ts: Date.now() };
}
      ...
      {open && !compact && (
        <div style={{ padding: "0 14px 14px" }}>
          ...
          {(slot.exercises || []).length > 0 && (
            <div style={{ background: "#F6F7F9", borderRadius: 12, padding: "9px 12px" }}>
              {slot.exercises.map((e, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, padding: "4px 0" }}>
                  <span className="f-body" style={{ fontSize: 13, color: "#374151", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.name}</span>
                  <span className="f-body tnum" style={{ fontSize: 12, color: "#9097A1", flexShrink: 0 }}>{shortDetail(e.detail)}</span>
                </div>
              ))}
              {canStart && <div className="f-body" style={{ fontSize: 11, color: "#B6BCC4", marginTop: 6 }}>Full targets, RPE & rest timers are in workout mode.</div>}
            </div>
          )}
...
function WeekDay({ dk, plan, isToday, isPast, accent, me, prefs, busy, aiOn, on }) {
  ...
  return (
    <Card style={{ padding: 0, overflow: "hidden",
      borderColor: isToday ? accent : isPast ? "#EEF0F3" : "#EBEEF2",
      borderWidth: isToday ? 2 : 1,
      background: isToday ? accent + "0a" : isPast ? "#FAFBFC" : "#fff",
      boxShadow: isToday ? "0 2px 10px " + accent + "1f" : "none" }}>
      <button onClick={() => setOpen(!open)} className="f-body"
        style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 15px", background: "none", border: "none", cursor: "pointer", textAlign: "left", opacity: isPast && !open ? 0.62 : 1 }}>
        <div style={{ width: 42 }}>
          <div className="f-disp" style={{ fontSize: 13, fontWeight: 700, color: isToday ? accent : isPast ? "#A6ACB5" : INK }}>{wname}</div>
          <div className="f-body" style={{ fontSize: 11, color: isPast ? "#B6BCC4" : "#9097A1" }}>{fmtDate(dk)}</div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          {slots.length === 0
            ? <span className="f-body" style={{ fontSize: 13, color: "#A6ACB5" }}>{exists ? "Rest day" : "—"}</span>
            : <div className="f-body" style={{ fontSize: 13, color: isPast ? "#7A828D" : INK, fontWeight: isToday ? 600 : 400, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {slots.map((s) => s.title).join("  ·  ")}
              </div>}
        </div>
        {isToday && <span className="f-body" style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: accent, padding: "2px 7px", borderRadius: 6, flexShrink: 0 }}>TODAY</span>}
        {isPast && allDone && <Check className="w-4 h-4" style={{ color: "#10B981", flexShrink: 0 }} />}
        {active.length > 0 && <span className="f-disp tnum" style={{ fontSize: 12, color: allDone ? "#10B981" : isPast ? "#B6BCC4" : "#9097A1", flexShrink: 0 }}>{done}/{active.length}</span>}
...
function Train({ me, week, prefs, onSaveDay, onChangeDay, onGenWeek, onPref, onStart, aiWeekBusy }) {
  ...
  return (
    <Screen>
      ...
      {dates.map((dk) => (
        <WeekDay key={dk} dk={dk} plan={plans[dk]} isToday={dk === today} isPast={dk < today} accent={accent} me={me} prefs={prefs} busy={busy} aiOn={aiOn} on={on} />
      ))}
      <RecoveryNote />
...
function StrengthMap({ lifts, bodyLb, accent, read, readBusy, onRead }) {
  ...
  return (
    <>
      ...
      {aiOn && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #EFF1F4" }}>
          {read ? (
            <>
              <div className="f-body" style={{ fontSize: 13, fontWeight: 700, color: INK, display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
                <Sparkles className="w-4 h-4" style={{ color: VIOLET }} /> Coach's read
              </div>
              <div className="f-body" style={{ fontSize: 13, color: "#374151", lineHeight: 1.6, marginBottom: 10 }}>{read.summary}</div>
              {[["Strengths", read.strengths, "#10B981"], ["Gaps", read.gaps, "#FB923C"], ["Do this", read.actions, VIOLET]].map(([label, items, col]) =>
                (items || []).length ? (
                  <div key={label} style={{ marginBottom: 9 }}>
                    <div className="f-body" style={{ fontSize: 10.5, fontWeight: 700, color: "#9097A1", letterSpacing: 0.4, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
                    {items.map((t, i) => (
                      <div key={i} className="f-body" style={{ fontSize: 12.5, color: "#4B5563", lineHeight: 1.55, display: "flex", gap: 7, marginBottom: 3 }}>
                        <span style={{ color: col, fontWeight: 700 }}>·</span><span>{t}</span>
                      </div>
                    ))}
                  </div>
                ) : null)}
              <button onClick={onRead} disabled={readBusy} className="f-body" style={{ background: "none", border: "none", padding: 0, cursor: readBusy ? "default" : "pointer", color: "#9097A1", fontSize: 12, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                <RefreshCw className="w-3.5 h-3.5" style={{ animation: readBusy ? "spin 1s linear infinite" : "none" }} /> {readBusy ? "Reading your log…" : "Refresh read"}
              </button>
            </>
          ) : (
            <>
              <Btn full kind="soft" color={VIOLET} size="sm" disabled={readBusy} onClick={onRead}>
                {readBusy ? <RefreshCw className="w-4 h-4" style={{ animation: "spin 1s linear infinite" }} /> : <Sparkles className="w-4 h-4" />} {readBusy ? "Reading your log…" : "Get the coach's read"}
              </Btn>
              <div className="f-body" style={{ fontSize: 11.5, color: "#9097A1", marginTop: 7, lineHeight: 1.5 }}>Sends your logged lifts to the AI coach for a deeper look — exercise selection, progression, and what you're not training.</div>
            </>
          )}
        </div>
      )}
...
function Progress({ me, data, busy, lifts, strengthRead, strengthBusy, onStrengthRead, onLogWeight, onLogMeasure, onAddPhoto, onDeletePhoto }) {
  ...
  return (
    <Screen>
      ...
      <Card>
        <StrengthMap lifts={lifts} bodyLb={+me.weightLb || 0} accent={accent} read={strengthRead} readBusy={strengthBusy} onRead={onStrengthRead} />
      </Card>
...
export default function App() {
  ...
  const [strengthRead, setStrengthRead] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved && obj[saved]) {
        ...
        setStrengthRead(await getJSON(`strength:${saved}`, true));
      }
      setBooted(true);
    })();
  ...
  async function handleSetupDone(profile, claimed) {
    ...
    setStrengthRead(await getJSON(`strength:${profile.id}`, true));
    setTab("today");
  }
  ...
  async function runStrengthRead() {
    if (strengthBusy || !me) return;
    setStrengthBusy(true);
    try {
      const { groups } = analyzeStrength(myLifts, +me.weightLb || 0);
      const r = await aiStrengthRead(me, myLifts, groups);
      await store.set(`strength:${meId}`, r, true);
      setStrengthRead(r);
    } catch (e) {
      setStrengthRead((prev) => prev || { summary: "Couldn't reach the coach just now — log another session or try again in a moment.", strengths: [], gaps: [], actions: [] });
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: INK }} className="f-body">
      ...
      {tab === "progress" && <Progress me={me} data={progress} busy={progressBusy} lifts={myLifts} strengthRead={strengthRead} strengthBusy={strengthBusy} onStrengthRead={runStrengthRead} onLogWeight={logWeight} onLogMeasure={logMeasure} onAddPhoto={addPhoto} onDeletePhoto={deletePhoto} />}
